...
- **좌표는 `get_screen_info`가 미리 계산해 맵에 저장**해두고, `tap_node`/`input_text`는 id로 그 좌표를 꺼내 제스처만 실행한다. 모델은 좌표를 직접 다루지 않는다.
- **`input_text`는 `node_id`로 입력 필드를 탭(포커스)한 뒤 `ACTION_SET_TEXT`로 주입**한다. (선택·입력을 한 도구 호출에서 수행; PokeClaw InputTextTool 핵심 경로와 동일)
