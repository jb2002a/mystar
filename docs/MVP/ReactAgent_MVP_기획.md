...
**상태:** ✅ MVP 완료 (2026-09-03, M5) · ✅ 포스트 MVP 완료 (2026-09-04, M6–M9) — 제네릭 도구 7개 + TTS + 웹 검색. 다음 확장은 [기획서 §7](./ReactAgent_MVP_기획.md#7-이후-확장-경로) (`ask_user` · 다단계 시나리오).
...
- **되돌릴 수 없는 행동에 안전장치.** 실제 사람에게 실제 메시지를 보낸다. MVP는 확인 없이 전송한다. 오발송 차단용 HITL은 M10 `ask_user`.
...
### 바로 다음 (포스트 MVP) — ✅ 완료 (M6–M9, 2026-09-04)

상세 설계·DoD·함정은 [포스트 MVP 로드맵](./ReactAgent_포스트MVP.md).
...
| M9 | `web_search` | 조회 목표는 구글을 열지 않고 검색 스니펫을 받는다. | ✅ |

### 그 다음 (포스트 MVP 잔여 + 확장)

| 단계 | 항목 | 한 줄 |
|---|---|---|
| M10 | `ask_user` | 정보 부족 질문 + 전송·결제 등 위험 탭 직전 HITL. |
| M11 | 다단계 테스트 | 기차 예매·회원가입·웹 조회 회귀. 실결제는 하지 않음. |

일일 회귀 6케이스는 [D0 골든셋](./ReactAgent_D0_골든셋.md).
