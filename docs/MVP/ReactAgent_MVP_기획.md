...
**상태:** ✅ MVP 완료 (2026-09-03, M5) — 음성 명령 → ReAct 루프 → 제네릭 도구 4개로 앱 자동 조작. HITL 전송 확인은 §7 이후 확장.
...
- **되돌릴 수 없는 행동에 안전장치.** 실제 사람에게 실제 메시지를 보낸다. MVP는 확인 없이 전송한다. 오발송 차단용 HITL 확인은 §7 이후 확장.
...
- **HITL 안전장치:** 전송·결제 등 되돌릴 수 없는 탭 직전에 사용자 확인.
